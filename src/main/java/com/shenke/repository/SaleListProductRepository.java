...
public interface SaleListProductRepository
	...
	*/
	 ...
	 */
	@Modifying
	@Query(value = "update t_sale_list_product set state = ?2,jitai_id = ?3,inform_number = ?4,issue_state = ?5 where id in ?1",nativeQuery = true)
	void jitaiAllocation(Integer []ids, String state, Integer id, Long informNumber, String iussueState);
	 ...
	 * @return
	 */
	@Query(value = "select * from t_sale_list_product where clientname = ?1 group by inform_number",nativeQuery = true)
    List<SaleListProduct> chaxunpeifang(String client);
}
